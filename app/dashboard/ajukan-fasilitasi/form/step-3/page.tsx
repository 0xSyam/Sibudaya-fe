...
export default function AjukanFasilitasiFormStep3Page() {
  ...
  function validate() {
    ...
    if (jenisId === 1) {
      ...
      else if (!/^\d{8,30}$/.test(nomorRekening.trim())) newErrors.nomorRekening = "Format nomor rekening tidak valid";
    ...
    try {
      ...
      if (pendingSertifikatNikFile) {
        if (!formData.nik?.trim()) {
          throw { message: "Nomor NIK belum lengkap. Kembali ke langkah identitas lembaga." };
        }
        if (!formData.nikTanggalTerbit || !formData.nikTanggalBerlakuSampai) {
          throw { message: "Tanggal sertifikat NIK belum lengkap. Kembali ke langkah identitas lembaga." };
        }
  ...
  sessionStorage.removeItem("sertifikat_file_name");
      ...
      const apiError = err as { message?: string | string[]; statusCode?: number };
  ...
  return (
    <section className="h-full overflow-y-auto px-4 pb-10 pt-8 sm:px-6 lg:pt-[84px]">
      <div className="mx-auto w-full max-w-[920px]">
        <FormPageHeader
          title={jenisId === 1 ? "Administrasi & Dokumen Pendukung" : "Dokumen Pendukung"}
          description={
            jenisId === 1
                ...
                onChange={(e) => {
                  ...
                  if (f) {
                    ...
                    if (validationMessage) {
                      ...
                      setErrors((p) => ({ ...p, proposalFile: validationMessage }));
