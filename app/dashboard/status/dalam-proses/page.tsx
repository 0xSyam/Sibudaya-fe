...
const timelineSteps: TimelineStep[] = [
  {
    ...
    description:
      "Data awal lembaga dan jenis fasilitas pentas telah berhasil diajukan.",
    status: "completed",
  },
  {
    ...
    status: "in_progress",
    hasButton: true,
    buttonLabel: "Lihat data Pengajuan",
  },
  {
...
export default function StatusDalamProsesPage() {
  return (
                    ...
                    {timelineSteps.map((step, index) => (
                      <div key={step.title} className="grid grid-cols-[32px_minmax(0,1fr)] gap-3 md:grid-cols-[160px_32px_minmax(0,1fr)] md:gap-5">
                        ...
                        <article className="rounded-[10px] bg-white p-4 shadow-[0_4px_14px_0_rgba(38,43,67,0.16)] md:p-5">
                          ...
                          {step.status === "in_progress" && step.hasButton && step.buttonLabel ? (
                            <Link
                              href="#"
                              className="mt-4 inline-flex h-[38px] items-center justify-center gap-2 rounded-[8px] bg-[#c23513] px-[22px] text-[15px] font-medium capitalize leading-[22px] text-white shadow-[0_2px_6px_0_rgba(38,43,67,0.14)] transition-colors hover:bg-[#a62c10]"
                            >
                              {step.buttonLabel}
                              <ArrowUpRightIcon />
                            </Link>
